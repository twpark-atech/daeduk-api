...
from .utils import rain_variables, prediction, result_to_geojson, result_to_geojsonDB, extract_rain_in_api
from .config import *
...
#     return result_to_geojsonDB(mask, TRANSFORM, CRS)

def _infer_and_geojson(rain_feat, batch_size: int, scnr_id: int, fcstdt: str) -> str:
    if not os.path.exists(FEATURE_PATH):
        raise FileNotFoundError(f"{FEATURE_PATH} not found")
    ...
    mask[_mask_array == 1] = 0
    return result_to_geojsonDB(mask, TRANSFORM, CRS, scnr_id, fcstdt)
...
@shared_task(name="simulation_task")
def simulation_task(rain: list[float], batch_size: int, scnr_id: int, fcstdt: str) -> str:
    rain_feat = rain_variables(rain)
    return _infer_and_geojson(rain_feat, batch_size, scnr_id, fcstdt)

@shared_task(name="realtime_task")
def realtime_task(forecast_hour: int, batch_size: int, scnr_id: int, fcstdt: str) -> str:
    rain = extract_rain_in_api(forecast_hour)
    rain_feat = rain_variables(rain)
    return _infer_and_geojson(rain_feat, batch_size, scnr_id, fcstdt)